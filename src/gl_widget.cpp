 ...
 */
void
gl_widget::mousePressEvent(QMouseEvent* e)
{
  int x = e->position().x();
  int y = e->position().y();
 ...
 */
void
gl_widget::mouseReleaseEvent(QMouseEvent* e)
{
  this->mouse.depressed = 0;
  return;
 ...
 */
void
gl_widget::mouseMoveEvent(QMouseEvent* e)
{
  int x = e->position().x();
  int y = e->position().y();
  char buf[64] = {0};
  ...
  if (this->selected_object && (this->selected_object->body_part == MD3_LIGHT))
  {
    ...
    if (g_world->light[0].trot > 360.0f)
      g_world->light[0].trot -= 360.0f;
    if (g_world->light[0].trot < 0.0f)
      g_world->light[0].trot += 360.0f;
    if (g_world->light[0].prot > 90.0f)
      g_world->light[0].prot = 90.0f;
    if (g_world->light[0].prot < -90.0f)
      g_world->light[0].prot = -90.0f;
  ...
  else
  {
    ...
    if (g_world->camera.trot > 360.0f)
      g_world->camera.trot -= 360.0f;
    if (g_world->camera.trot < 0.0f)
      g_world->camera.trot += 360.0f;
    if (g_world->camera.prot > 90.0f)
      g_world->camera.prot = 90.0f;
    if (g_world->camera.prot < -90.0f)
      g_world->camera.prot = -90.0f;

    /* update GUI widget with camera position */
    sprintf(buf, "Camera Position:   x = %3.2f   y = %3.2f   z = %3.2f", g_world->camera.r * cos(g_world->camera.prot * deg) * cos(g_world->camera.trot * deg), g_world->camera.r * sin(g_world->camera.prot * deg), g_world->camera.r * cos(g_world->camera.prot * deg) * sin(g_world->camera.trot * deg));
...
void
gl_widget::select_object(int x, int y)
{
